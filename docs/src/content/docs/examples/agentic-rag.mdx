...
|------|-----------|---------|
| `clone_skill` | `tenant_id`, `skill_name` | Copy a base skill's handler source as a starting point |
| `customize_skill` | `tenant_id`, `skill_name`, `source_code`, `auto_promote=False`, `meta_overrides=None` | Submit modified handler source; validates and stores the variant |
| `promote_variant` | `tenant_id`, `skill_name`, `variant_id` | Make a variant the active handler for the tenant's skill |
| `demote_variant` | `tenant_id`, `skill_name`, `variant_id` | Move a promoted variant back to sandbox status |
| `preview_variant` | `tenant_id`, `skill_name`, `variant_id` | Route real tenant calls to the variant without full promotion |
| `run_variant` | `tenant_id`, `skill_name`, `variant_id`, `input_data` | Test a variant with input (sandbox if provisioned, else in-process) |
| `get_variant_source` | `tenant_id`, `skill_name`, `variant_id` | Return a variant's handler source |
| `list_tenant_skills` | `tenant_id` | List all variants (sandbox + promoted) for a tenant |
| `provision_sandbox` | `tenant_id`, `skills_dir=""` | Provision an isolated subprocess sandbox; returns its endpoint URL |
| `teardown_sandbox` | `tenant_id` | Tear down a tenant's sandbox |
| `sandbox_health` | `tenant_id` | Check a tenant's sandbox health |
| `push_to_sandbox` | `tenant_id`, `skill_name` | Push the promoted variant's handler source to the sandbox |
