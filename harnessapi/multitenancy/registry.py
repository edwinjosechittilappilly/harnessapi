...
class TenantSkillRegistry:
    ...
    def get_variant(self, variant_id: str) -> SkillVariant | None:
        """Look up any variant (promoted or sandbox) by variant_id."""
    ...
    def remove_variant(self, variant_id: str) -> bool:
        """Remove a variant from both indexes. Returns True if found."""
        removed = False
    ...
    def list_all_for_tenant(self, tenant_id: str) -> list[SkillVariant]:
        return self.list_promoted_for_tenant(tenant_id) + self.list_sandbox_for_tenant(tenant_id)
